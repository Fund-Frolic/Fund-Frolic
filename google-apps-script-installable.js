 ...
 * INSTALLABLE TRIGGER VERSION - Required for sending emails
 *
 ...
 * 5. Update the CONFIG section below with your email
 * 6. Click Save (disk icon)
 * 7. Follow the trigger setup instructions below!
 */
...
function setupTrigger() {
  ...
  SpreadsheetApp.getUi().alert('✅ Trigger installed successfully! Emails will now be sent automatically.');
 ...
 * DO NOT rename this function - it's connected to the trigger
 */
function onSheetEdit(e) {
  try {
    ...
    // Log but don't throw - we don't want to break the trigger
  }
}
...
function createGrantResultsUserEmail(name, grants) {
  ...
  grants.forEach(function(grant, index) {
    const eligibilityList = grant.eligibility ? grant.eligibility.split(';').map(function(e) { return '<li>' + e.trim() + '</li>'; }).join('') : '';
...
function createGrantResultsClientEmail(name, email, orgName, phone, grants, projectDesc, revenueStatus, orgType, searchId) {
  let grantsHtml = grants.map(function(grant, index) {
    return `
