 ...
 *
 * This script automatically sends emails when contact information is submitted.
 * It detects two scenarios:
 * 1. Standalone contact (no grant search) - sends simple thank you email
 * 2. Grant results contact (with search data) - sends email with 3 grant matches
 *
 ...
 * 7. Click "Run" > "Run function" > "createTrigger" to set up automatic trigger
 * 8. Grant permissions when prompted
 *
 * The script will now run automatically whenever contact info is added!
 */
...
function onEdit(e) {
  try {
    // Only proceed if the edit was in the ContactSubmitted column (Y)
    const range = e.range;
    const sheet = range.getSheet();
    const col = range.getColumn();
    const row = range.getRow();

    // Column Y is column 25 (1-based index)
    if (col !== 25) return;
    ...
    processContactSubmission(rowData);
...
/**
 * Create the trigger (run this once manually)
 */
 ...
 */
function processContactSubmission(rowData) {
